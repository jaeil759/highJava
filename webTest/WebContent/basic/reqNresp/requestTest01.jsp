...
<body>
	...
	<%--
	 ...
	 --%>
	 <%
	 	//이영역은 JSP문서에서 JAVA명령을 사용할 수 있는 영역 '스크립트릿' 이라고 부름
	 	String name = "홍길동";
	...
	<!-- 
		<form태그의 속성
		1) action => <form>에서 구성한 데이털ㄹ 서버로 전송할 때 이 데이터를 받아서 처리할 문서명 또는 Servlet URL
					 이 속성이 생략되면 현재 문서가 처리할 문서가 된다.
		2) method ==> 전송방식 (GET 또는 POST) ==> (기본 : GET)
		3) enctype ==> 서버로 파일을 전송할 때 사용하는 속성으로 'multipart/fom-data' 값을 지정한다.
		4) target ==> 응답 결과가 나타날 프레임 명 
	 -->
