...
/*
 * - 서블릿3.0 이상에서 파일 업로드를 하려면 서브릿에 @MultipartConfig 애노테이션을 설정해야한다.
 * - @multipartConfig 애노테이션의 설정 변수를
 ...
 * 4) maxRequestSize : 서버로 전송되는 전체 Request객체의 최대 크기( 모든 파일크기 + formData)
 * 					(단위 : byte, 기본값 : -1L(무제한))
 * 
 *  
 */
...
public class FileUploadController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {
				...
				fvo.setFile_writer(userName);		// 작성자 저장
				fvo.setOrigin_file_name(fileName);  // 실제 파일명 저장
				...
				// part객체.write() ==> upload된 파일을 지정한 경로위치에 저장하는 메서드
				part.write(uploadPath + File.separator + saveFileName);
				
				fileList.add(fvo);	// upload된 하나의 파일 정보를 List에 추가
			}//if문끝
		}//for문끝
	 ...
	 * - Part 구조
	 * 1) 파일이 아닌 일반 데이터일 경우
	 * -----------------------123123213asdasd12312asd132 ==> Part를 구분하는 구분선
