...
public class RequestTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setCharacterEncoding("utf-8"); // POST 방식으로 전달되는 문자의 인코딩 방식 지정
		
		// 클라이언트가 보내온 데이터 받기
		
		// 1) request객체.getParameter("파라미터명"); ==> 해당 '파라미터명' 에 설정된 '값'을 가져온다.
		// 				==> 가져오는 '값'의 자료형은 'String; 이다.
		//				==> '파라미터명'은 <form>태그 안에서 지정한 객체의 name속성값을 말한다.
		String userName = request.getParameter("username");
		String job = request.getParameter("job");
		
		// 2) request객체.getParameterValues("파라미터명") ==> '파라미터명'이 같은것이 여러개 일 경우에 사용한다.
//					==> 가져오는 '값'의 자료형은 'String[]' 이다.
		// getParameterValues()메서드를 이용하여 name속성값이 'hobby'로 설정된 체크박스의 값들을 읽어오기
		...
		out.println("<h2>Request객체의 메서드 처리결과</h2>");
		out.println("<ol>");
		...
		for(String paramName : paramsMap.keySet()) { // 파라미터명의 개수만큼 반복
			String[] paramValues = paramsMap.get(paramName); // 파라미터 Value값 구하기
		...
		//		이 Map객체의 key값은 '파라미터명' 이고, 자료형은 'String'이고,
		//		value값의 자료형은 'String[]'이다.
